...
const { getUserList, createUser } = useUserListStore()
...
async function showUser(user: any){
        ...
        <div class="inline-block min-w-full align-middle">
          <div class="overflow-hidden shadow sm:rounded-lg">
            <div class="p-1 flex items-center justify-between">
              <div></div>
              <!-- Modal toggle -->
              <button @click="handleShowModal('create')" type="button"
                ...
                <div class="relative p-4 w-full max-w-md max-h-full">
                  ...
                  <div class="relative bg-white rounded-lg shadow-lg dark:bg-[#2F3349FF]">
                    ...
                    <div v-if="error" class="p-3 md:p-2">
                      <div class="flex p-2 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                    ...
                    <form @submit.prevent="submit" class="max-w-sm py-4 mx-auto">
                      ...
                      <div class="mb-5 relative">
                        ...
                        <input :type="togglePassword.password_confirm ? 'password' : 'text'" v-model="form.password_confirmation" id="repeat-password"
                          class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-[#2F3349FF] dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                          required />
                        <button type="button" @click="togglePassword.password_confirm = !togglePassword.password_confirm" class="absolute top-[34px] right-2 ">
                          <svg v-if="!togglePassword.password_confirm" xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M12 16q1.875 0 3.188-1.312T16.5 11.5q0-1.875-1.312-3.187T12 7q-1.875 0-3.187 1.313T7.5 11.5q0 1.875 1.313 3.188T12 16m0-1.8q-1.125 0-1.912-.788T9.3 11.5q0-1.125.788-1.912T12 8.8q1.125 0 1.913.788T14.7 11.5q0 1.125-.787 1.913T12 14.2m0 4.8q-3.65 0-6.65-2.037T1 11.5q1.35-3.425 4.35-5.462T12 4q3.65 0 6.65 2.038T23 11.5q-1.35 3.425-4.35 5.463T12 19m0-2q2.825 0 5.188-1.487T20.8 11.5q-1.25-2.525-3.613-4.012T12 6Q9.175 6 6.813 7.488T3.2 11.5q1.25 2.525 3.613 4.013T12 17"/></svg>
                          <svg v-else xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="m16.1 13.3l-1.45-1.45q.225-1.175-.675-2.2t-2.325-.8L10.2 7.4q.425-.2.863-.3T12 7q1.875 0 3.188 1.313T16.5 11.5q0 .5-.1.938t-.3.862m3.2 3.15l-1.45-1.4q.95-.725 1.688-1.587T20.8 11.5q-1.25-2.525-3.588-4.012T12 6q-.725 0-1.425.1T9.2 6.4L7.65 4.85q1.025-.425 2.1-.638T12 4q3.775 0 6.725 2.087T23 11.5q-.575 1.475-1.513 2.738T19.3 16.45m.5 6.15l-4.2-4.15q-.875.275-1.762.413T12 19q-3.775 0-6.725-2.087T1 11.5q.525-1.325 1.325-2.463T4.15 7L1.4 4.2l1.4-1.4l18.4 18.4zM5.55 8.4q-.725.65-1.325 1.425T3.2 11.5q1.25 2.525 3.588 4.013T12 17q.5 0 .975-.062t.975-.138l-.9-.95q-.275.075-.525.113T12 16q-1.875 0-3.188-1.312T7.5 11.5q0-.275.038-.525t.112-.525zm4.2 4.2"/></svg>
                        </button>
                      </div>
                      ...
                      <div class="mb-5">
                        ...
                        <select id="countries" v-model="form.roles" multiple
                          ...
                          <option v-for="(item, index) in roleLists " :selected="form.roles.find(el=>el === item.id)" :key="index" :value="item.id">
                      ...
                      <button type="submit" class="text-white active-link focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center ">
                        {{ $t('register-new-user') }}
                      </button>
                    </form>
                  </div>
    ...
    <div class="flex items-center justify-center pt-1 sm:pt-2">
      <nav aria-label="Page navigation example">
        <ul class="inline-flex -space-x-px text-sm">
          <li>
            <button :disabled="!userLists['prev_page_url']" :key="5" @click="prevNextPage(userLists['prev_page_url'])" class="flex items-center justify-center px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-black dark:bg-[#2F2B3DFF] dark:border-gray-700 dark:text-white dark:hover:bg-[#2F3349FF] dark:hover:text-white">
              &laquo; {{ $t('previous') }}
            </button>
          </li>
          <li v-for="page in userLists['last_page']" :key="page">
            <button @click="changePage(page)" :class="userLists['current_page'] === page ? 'active-link text-white' : 'text-black'" class="flex items-center justify-center px-3 h-8 leading-tight bg-white border border-gray-300 hover:bg-gray-100 hover:text-black dark:bg-[#2F2B3DFF] dark:border-gray-700 dark:text-white dark:hover:bg-[#2F3349FF] dark:hover:text-white">
              {{ page }}
            </button>
          </li>
          <li >
            <button :disabled="!userLists['next_page_url']" :key="6" @click="prevNextPage(userLists['next_page_url'])" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700 dark:bg-[#2F2B3DFF] dark:border-gray-700 dark:text-white dark:hover:bg-[#2F3349FF] dark:hover:text-white">
              {{ $t('next') }} &raquo;
            </button>
          </li>
        </ul>
