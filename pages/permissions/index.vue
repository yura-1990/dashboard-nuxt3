...
async function createPermissions(){
  await createPermission(permissionForm.value.descriptions)
  await getPermissionList()
...
const links = ref([
  {
    title: 'permissions',
    icon: { name: 'arcticons:permissionchecker', size: '16' },
    to: '/permissions',
  }
])
  ...
  <div>
    <div class="grid gap-4 xl:grid-cols-2 2xl:grid-cols-3 relative">
      <div class=" 2xl:col-span-2 relative">
        <div class="my-3 flex sticky top-[-5px] backdrop z-50 justify-between items-center">
          <div class="">
        ...
        <div class="dark:bg-[#2F3349FF] bg-white border border-gray-500 rounded-lg shadow-sm dark:border-gray-500 overflow-hidden p-1">
          <table class="w-full  border-collapse text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            ...
            <tbody class="dark:bg-[#2F3349FF] odd:bg-white odd:dark:bg-[#2F3349FF] even:bg-[#2F3349FF] even:dark:bg-[#2F3349FF] border-b dark:border-gray-700">
              <tr v-for="(item, index) in permissionLists" :key="index"
      ...
      <div class="p-4 bg-white sticky top-[65px] border border-gray-200 mt-4 rounded-lg h-80 shadow-sm dark:border-gray-700 sm:p-6 dark:bg-[#2F3349FF]">
        <form>
          <div class="mb-6">
            <div v-for="(item, index) in locales" :key="index" class="flex items-center gap-2" >
              ...
              <input type="text"
                     ...
                     required
                     v-model="permissionForm.descriptions[item.code]"
              />
            </div>
          </div>
          <div>
